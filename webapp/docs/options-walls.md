...
## OI 缺失时的成交量代理(basis 自动切换)

Yahoo 的 `openInterest` 由 OCC **隔夜结算**,近月/新上市合约常出现 **OI=0 但成交量(volume)很大**的情况(例:NVDA 近月)。
若仍只按 OI 画墙,会出现「明明成交活跃却看不到墙」。

因此后端做了**依据自动切换**:
- 当 `总OI < 总成交量 × 10%`(视为 OI 缺失/严重滞后)→ `basis="volume"`,
  **墙 / Max Pain / GEX / 分布图**全部改用**成交量**作权重(更反映当日新建仓,但非持仓累计)。
- 否则 `basis="oi"`,按未平仓量计算(标准口径)。
- 响应里返回 `basis` 字段,前端据此切换图例(Call OI ↔ Call Vol)并显示提示横幅。

> 成交量代理是"当日资金流"视角,与 OI 的"持仓累计"视角互补;真实环境 OI 正常时仍优先用 OI。
